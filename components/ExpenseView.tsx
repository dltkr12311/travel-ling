...
import { Wallet, Plus, Trash2, Copy, Check, X, ChevronRight, User, PiggyBank, AlertTriangle, TrendingUp } from 'lucide-react';
...
const ExpenseView: React.FC<Props> = ({ people, expenses, onAddPerson, onAddExpense, onRemoveExpense, budget, onSetBudget }) => {
  const [isAddingExpense, setIsAddingExpense] = useState(false);
  const [newExpenseAmount, setNewExpenseAmount] = useState('');
  const [newExpenseDesc, setNewExpenseDesc] = useState('');
  const [newExpensePayer, setNewExpensePayer] = useState<string>(people[0]?.id || '');
  ...
  // 1. 총 지출 계산
  const totalSpent = useMemo(() => expenses.reduce((acc, cur) => acc + cur.amount, 0), [expenses]);
  ...
  const transferInstructions = useMemo(() => {
      const debtors = settlement.filter(p => p.balance < -10).map(p => ({ ...p, debt: Math.abs(p.balance) }));
      const creditors = settlement.filter(p => p.balance > 10).map(p => ({ ...p, credit: p.balance }));
  ...
  return (
    <div className="pb-32 bg-[#f2f4f6] min-h-full">
      ...
      {budget === 0 ? (
          <div className="bg-white p-6 pt-8 pb-8 rounded-b-3xl shadow-sm">
              <div className="text-center mb-6">
                  ...
                  <h2 className="text-xl font-black text-slate-900 mb-2">얼마나 쓰실 계획인가요?</h2>
                  <p className="text-slate-500 text-sm">예산을 등록하면 AI가<br/>과소비를 막아드릴게요.</p>
              </div>
              ...
              <button 
                onClick={() => tempBudget && onSetBudget(parseInt(tempBudget.replace(/,/g, '')))}
                disabled={!tempBudget}
          ...
          <div className={`p-6 pt-8 pb-8 rounded-b-3xl shadow-sm transition-colors duration-500 ${health.color}`}>
              {/* Header: Edit Button */}
              <div className="flex justify-between items-start mb-6">
                  <div className={`text-xs font-bold opacity-80 px-2.5 py-1 rounded-full border flex items-center gap-1 ${health.textColor.includes('white') ? 'border-white/30 text-white' : 'border-slate-200 text-slate-500'}`}>
                      {health.icon}
                      <span>소비 리포트</span>
                  </div>
                  <button onClick={() => onSetBudget(0)} className={`text-xs underline opacity-60 hover:opacity-100 ${health.textColor.includes('white') ? 'text-white' : 'text-slate-400'}`}>
                      예산 수정
                  </button>
              </div>
              ...
              {/* Main Amount Display */}
              <div className="mb-1">
                  <p className={`text-sm font-bold mb-1 opacity-80 ${health.textColor.includes('white') ? 'text-blue-100' : 'text-slate-500'}`}>남은 예산</p>
                  <div className={`flex items-baseline gap-1 ${health.textColor.includes('white') ? 'text-white' : 'text-slate-900'}`}>
              ...
              <div className="mt-6 mb-4">
                  <div className="flex justify-between text-[11px] font-bold mb-1.5 opacity-70">
                      <span className={health.textColor.includes('white') ? 'text-white' : 'text-slate-500'}>
                  ...
                  <div className="h-3 w-full bg-black/10 rounded-full overflow-hidden backdrop-blur-sm">
                      <div 
                        className={`h-full rounded-full transition-all duration-1000 ease-out ${health.status === 'critical' ? 'bg-red-500' : health.status === 'danger' ? 'bg-red-500' : health.status === 'warning' ? 'bg-orange-400' : 'bg-white'}`}
              ...
              <div className={`mt-4 p-4 rounded-xl backdrop-blur-md border flex flex-col gap-1 ${health.textColor.includes('white') ? 'bg-white/10 border-white/20 text-white' : 'bg-white border-slate-100 shadow-sm'}`}>
                  <p className={`font-bold text-base ${health.textColor.includes('white') ? 'text-white' : 'text-slate-800'}`}>{health.msg}</p>
                  <p className={`text-xs ${health.textColor.includes('white') ? 'text-blue-100' : 'text-slate-500'}`}>{health.sub}</p>
              </div>
              ...
              <div className="mt-6 flex items-center justify-between">
                <div className="flex -space-x-2">
                    {people.map((p, idx) => (
                        <div key={p.id} className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-[10px] font-bold uppercase z-0 relative ${health.textColor.includes('white') ? 'border-blue-500 bg-blue-800 text-white' : 'border-white bg-slate-200 text-slate-600'}`} style={{zIndex: people.length - idx}}>
                            {p.name.slice(0, 2)}
                        </div>
                    ))}
                    <button 
                        onClick={() => setIsAddingPerson(true)}
                        className={`w-8 h-8 rounded-full border-2 border-dashed flex items-center justify-center z-10 ${health.textColor.includes('white') ? 'border-blue-300/50 text-blue-100 hover:bg-white/10' : 'border-slate-300 text-slate-400 hover:bg-slate-100'}`}
                    >
                        <Plus size={14} />
                    </button>
                </div>
                <div className={`text-[11px] font-bold opacity-80 ${health.textColor.includes('white') ? 'text-blue-100' : 'text-slate-400'}`}>
            ...
            {isAddingPerson && (
                 <div className="mt-3 flex items-center gap-2 animate-in slide-in-from-top-2">
                    ...
                    />
                    <button onClick={() => setIsAddingPerson(false)} className="p-2 bg-black/20 rounded-full text-white"><X size={12}/></button>
                </div>
            )}
          </div>
      )}
      ...
      {transferInstructions.length > 0 && (
          <div className="px-5 mt-6">
              <h3 className="text-slate-800 font-bold text-lg mb-3 ml-1">정산 알림</h3>
              <div className="grid gap-3">
                  {transferInstructions.map((inst, idx) => (
                      <div key={idx} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100/50 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                              <div className="bg-slate-100 text-slate-600 px-2 py-1 rounded-lg text-[10px] font-bold">송금</div>
                              <div className="text-slate-700 font-bold text-sm">
                                  {inst.from} <span className="text-slate-300 font-normal mx-1">→</span> {inst.to}
                              </div>
                          </div>
      ...
      <div className="px-5 mt-8">
        ...
        <div className="space-y-3">
            ...
            {expenses.slice().reverse().map(expense => {
                ...
                return (
                    <div key={expense.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100/50 flex justify-between items-center active:scale-[0.98] transition-transform">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <p className="font-bold text-slate-800 text-[15px]">{expense.description}</p>
                                <p className="text-xs text-slate-400 font-medium">{payer?.name} 결제</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <div className="font-bold text-slate-800 text-[15px]">{expense.amount.toLocaleString()}원</div>
                            <button onClick={() => onRemoveExpense(expense.id)} className="text-slate-300 hover:text-red-500 text-xs mt-1 p-1">
                                삭제
                            </button>
                        </div>
                    </div>
      ...
      {isAddingExpense && (
          <div className="fixed inset-0 z-50 flex items-end justify-center">
              <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setIsAddingExpense(false)}></div>
              <div className="bg-white w-full max-w-md rounded-t-3xl p-6 z-10 animate-in slide-in-from-bottom-10 duration-300 shadow-2xl pb-safe">
                  ...
                  <div className="mb-8 text-center">
                      <p className="text-slate-500 text-sm font-bold mb-2">얼마를 썼나요?</p>
                      <div className="flex justify-center items-center gap-1">
                        <input 
                            ...
                            onChange={e => setNewExpenseAmount(formatCurrency(e.target.value))}
                        />
                  ...
                  <div className="space-y-4">
                      <div>
                          <label className="block text-xs font-bold text-slate-400 mb-2 ml-1">누가 냈나요?</label>
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-400 mb-2 ml-1">무엇을 샀나요?</label>
                        <input 
