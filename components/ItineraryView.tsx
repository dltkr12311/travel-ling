...
import { resolveLocation, searchPlaces } from '../services/geminiService';
import { MapPin, Utensils, Bed, Car, Mountain, Plus, Trash2, GripVertical, X, Navigation, Search, Loader2 } from 'lucide-react';
...
const ItineraryView: React.FC<Props> = ({ items, onAddItem, onRemoveItem, onReorder }) => {
  ...
  const updateMapMarkers = () => {
      ...
      items.forEach((item, index) => {
          if (item.lat && item.lng) {
              ...
              const markerHtml = `
                <div class="transition-all duration-300 ${isActive ? 'scale-125 z-[1000]' : 'scale-100'}">
                    <div class="w-6 h-6 rounded-full ${isActive ? 'bg-blue-600' : 'bg-slate-700'} border-2 border-white shadow-lg flex items-center justify-center text-white font-bold text-[10px]">
                        ${index + 1}
                    </div>
                    ${isActive ? `<div class="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-blue-600 rotate-45"></div>` : ''}
                </div>
              `;
  ...
  // 4. Intersection Observer to track scrolling
  useEffect(() => {
      const observer = new IntersectionObserver((entries) => {
          ...
          rootMargin: '-10% 0px -30% 0px' // Adjust sensing area to center
      });
  ...
  const handleAdd = async () => {
    if (newItem.title) {
      // Use existing lat/lng if selected from search, otherwise resolve
      let lat = newItem.lat;
      let lng = newItem.lng;
      let cleanName = newItem.location || newItem.title;

      if (!lat || !lng) {
          try {
            const coords = await resolveLocation(newItem.title);
            lat = coords.lat;
            lng = coords.lng;
            cleanName = coords.cleanName;
          } catch (e) {
  ...
  const getPlaceIcon = (type: string) => {
      switch(type) {
          ...
          case 'activity': return <Mountain size={20} className="text-white" />;
          case 'hotel': return <Bed size={20} className="text-white" />;
          case 'travel': return <Car size={20} className="text-white" />;
  ...
  const getPlaceColor = (type: string) => {
      switch(type) {
          ...
          case 'travel': return 'bg-slate-500';
  ...
  return (
    <div className="flex flex-col h-full relative bg-white overflow-hidden">
      ...
      >
        {/* Handle bar */}
        <div className="sticky top-0 bg-white z-20 pt-3 pb-2 flex justify-center shrink-0" onClick={() => setIsAdding(true)}>
        ...
        <div className="px-5 pb-2 flex justify-between items-center sticky top-6 bg-white z-20 mb-1 shrink-0">
             <div>
                 ...
                 <p className="text-[11px] text-slate-400 font-medium">12월 12일 (금)</p>
             </div>
             <button 
             ...
             {items.map((item, index) => {
                ...
                return (
                    <div 
                        ...
                        className={`group relative flex gap-3 mb-3 z-10 transition-opacity duration-300 ${isActive ? 'opacity-100' : 'opacity-60'}`}
                        draggable
                        ...
                        onClick={() => {
                            setActiveId(item.id);
                            if (item.lat && item.lng && mapInstance.current) {
                                mapInstance.current.flyTo([item.lat, item.lng], 15, { animate: true });
                            }
                        }}
                    >
                        {/* Marker Badge in List */}
                        <div className="flex flex-col items-center pt-0.5 min-w-[2rem]">
                            <div className={`w-6 h-6 rounded-full ${isActive ? 'bg-blue-600 scale-105' : 'bg-slate-400'} text-white text-[10px] font-bold shadow-sm border border-white flex items-center justify-center z-10 transition-all`}>
                                {index + 1}
                            </div>
                            <div className="text-[9px] font-bold text-slate-300 mt-0.5 tracking-tighter">{item.time}</div>
                        </div>
                        
                        {/* Card */}
                        <div className={`flex-1 p-3 rounded-xl border transition-all cursor-pointer ${isActive ? 'bg-white border-blue-200 shadow-md ring-1 ring-blue-50' : 'bg-white border-slate-100 shadow-sm'}`}>
                            <div className="flex justify-between items-start gap-2">
                                <div className="flex-1 min-w-0">
                                    <h3 className={`font-bold text-[15px] leading-tight truncate ${isActive ? 'text-slate-900' : 'text-slate-600'}`}>{item.title}</h3>
                                    <div className="flex items-center gap-1 mt-0.5 text-[11px] text-slate-400">
                                        <span className="truncate font-medium">{item.type === 'food' ? '식사' : item.type === 'activity' ? '활동' : item.type === 'hotel' ? '숙소' : '이동'}</span>
      ...
      {isAdding && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center">
             <div className="absolute inset-0 bg-black/60 backdrop-blur-sm transition-opacity" onClick={() => setIsAdding(false)}></div>
             <div className="bg-white w-full max-w-md rounded-t-3xl p-0 z-10 animate-in slide-in-from-bottom-10 duration-300 pb-safe shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <div className="px-6 pt-6 pb-4">
                     ...
                     <h2 className="text-2xl font-black text-slate-900 mb-1">새로운 일정</h2>
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar px-6 pb-6">
                    ...
                    {searchResults.length > 0 ? (
                        <div className="mb-8 space-y-2 animate-in slide-in-from-bottom-2 fade-in">
                            <label className="block text-xs font-bold text-slate-400 mb-2 ml-1">검색 결과</label>
                            {searchResults.map((place, idx) => (
                                ...
                                >
                                    ...
                                    <div className={`w-12 h-12 rounded-xl ${getPlaceColor(place.type)} shadow-sm flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform`}>
                                        {getPlaceIcon(place.type)}
                                    </div>
                                    <div className="min-w-0">
                                        <div className="font-bold text-slate-800 text-[15px] truncate">{place.name}</div>
                                        <div className="text-xs text-slate-400 truncate mt-0.5">{place.address}</div>
                                        {place.description && <div className="text-[10px] text-blue-500 mt-1 truncate">{place.description}</div>}
                                    </div>
                                </button>
                            ))}
                        </div>
                         ...
                         <div className="mb-8 text-center py-4">
                            <p className="text-slate-400 text-sm">검색 결과가 없어요.<br/>직접 입력하시겠어요?</p>
                         </div>
                    ) : null}
                    ...
                    <div className="space-y-5 border-t border-slate-100 pt-6">
                        <div className="flex gap-3">
                            <div className="flex-1">
                                <label className="block text-xs font-bold text-slate-400 mb-1.5 ml-1">시간</label>
                                <input 
                                    type="time" 
                             ...
                             <div className="flex-[1.8]">
                                 <label className="block text-xs font-bold text-slate-400 mb-1.5 ml-1">유형</label>
                                 <div className="flex gap-1 bg-slate-50 p-1 rounded-2xl">
                                    {['food', 'activity', 'hotel', 'travel'].map(t => (
                                         <button
                                            ...
                                            className={`flex-1 h-10 rounded-xl flex items-center justify-center transition-all ${newItem.type === t ? 'bg-white shadow-sm ring-1 ring-black/5' : 'text-slate-300 hover:text-slate-500'}`}
                                         >
                                             {t === 'food' && <Utensils size={14} className={newItem.type === t ? "text-orange-500" : ""}/>}
                                             {t === 'activity' && <Mountain size={14} className={newItem.type === t ? "text-emerald-500" : ""}/>}
                                             {t === 'hotel' && <Bed size={14} className={newItem.type === t ? "text-indigo-500" : ""}/>}
                                             {t === 'travel' && <Car size={14} className={newItem.type === t ? "text-slate-600" : ""}/>}
                                         </button>
                                     ))}
                                 </div>
                             </div>
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-400 mb-1.5 ml-1">장소명 (직접 수정 가능)</label>
                            <input 
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-400 mb-1.5 ml-1">메모</label>
                             <input 
