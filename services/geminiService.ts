...
import { WeatherInfo, PlaceSearchResult } from "../types";
...
export const resolveLocation = async (locationName: string): Promise<{ lat: number; lng: number; cleanName: string }> => {
  try {
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            cleanName: { type: Type.STRING, description: "Official Korean name of the place" }
          }
        }
      }
    });
    ...
    return { lat: 38.2070, lng: 128.5918, cleanName: locationName };
  }
};
...
export const getSokchoWeatherAndSunrise = async (): Promise<WeatherInfo> => {
  try {
    const response = await ai.models.generateContent({
      model: MODEL_NAME,
      contents: "Predict the weather for Sokcho, South Korea on the morning of December 13, 2025. Specifically focusing on Seoraksan Ulsanbawi sunrise hike. Include sunrise time, expected temperature at 6AM, and wind chill. Provide hiking advice in Korean. Return ONLY valid JSON.",
      config: {
        responseMimeType: "application/json",
        responseSchema: {
          type: Type.OBJECT,
          properties: {
            ...
            condition: { type: Type.STRING, description: "Weather condition in Korean (e.g., 맑음, 흐림)" },
            windSpeed: { type: Type.STRING },
            hikingAdvice: { type: Type.STRING, description: "Short hiking advice in Korean" }
          }
        }
      }
    });
    ...
    return {
      ...
      condition: "구름 조금",
      windSpeed: "5 m/s",
      hikingAdvice: "울산바위 정상은 바람이 매우 강하니 체감 온도는 -8도 이하일 수 있습니다. 아이젠과 방풍 자켓을 꼭 챙기세요."
    };
  }
};

export const getTravelSuggestions = async (query: string, locationContext: string = "Sokcho, South Korea"): Promise<{ text: string; mapLinks?: { uri: string; title: string }[] }> => {
  try {
    const response = await ai.models.generateContent({
      model: MODEL_NAME,
    ...
    return { text: "지금은 AI 여행 비서와 연결할 수 없어요. 잠시 후 다시 시도해주세요.", mapLinks: [] };
  }
};
...
export const generatePackingList = async (): Promise<string[]> => {
    try {
        const response = await ai.models.generateContent({
            model: MODEL_NAME,
            contents: "Generate a checklist of 10 essential items for a winter trip to Sokcho (Dec 12-13) including Ulsanbawi hiking. Return the items in Korean. Return JSON array of strings.",
            config: {
                responseMimeType: "application/json",
                responseSchema: {
                    type: Type.ARRAY,
        ...
        return ["핫팩", "등산화", "히트텍/내복", "카메라", "비니/모자", "바람막이", "장갑", "보온병", "보조배터리", "선글라스"];
    }
}